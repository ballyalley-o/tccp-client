...
import {
  AssignmentProgress as ProgressIcon,
    ...
    <Stack spacing={4}>
      ...
      <Box>
        <Typography variant="h1" sx={{ mb: 1 }}>
          {transl('message.welcome') || 'Welcome back'}, {firstName}! 👋
        </Typography>
        <Typography variant="body1" color="text.secondary">
          {isStudent
            ? 'Continue your learning journey and explore new bootcamps'
              ...
              {isStudent && (
                <>
                  <Button
                    ...
                    onClick={() => navigate(PATH.BOOTCAMP.ROOT)}
                    sx={{ flex: 1 }}
                  >
                    Browse Bootcamps
                  </Button>
                  <Button
                    ...
                    onClick={() => navigate(PATH.COURSE.ROOT)}
                    sx={{ flex: 1 }}
                  >
                    Browse Courses
                  </Button>
                  <Button
                    variant="outlined"
                    startIcon={<MoreIcon />}
                    onClick={() => navigate(PATH.AUTH.ACCOUNT.ROOT)}
                    sx={{ flex: 1 }}
                  >
                    View Profile
                  </Button>
      ...
      <Box>
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
          <Typography variant="h2">
            {isStudent ? 'Your Current Courses' : 'Your Courses'}
          </Typography>
          <Button
            variant="text"
            endIcon={<NextIcon />}
      ...
      <Box>
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
          <Box>
            <Typography variant="h2">
              {isStudent ? 'Featured Bootcamps' : 'Popular Bootcamps'}
            </Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
              {isStudent
                ? 'Discover top-rated bootcamps in your area'
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                {isStudent
                  ...
                  : 'Content popular in your teaching areas'}
              </Typography>
            </Box>

            <Stack spacing={1.5}>
              {['Full Stack Web Development', 'Mobile App Development', 'Cloud & DevOps'].map(
                (rec, idx) => (
                    ...
                    <CardContent>
                      ...
                      >
                        <Box>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            {isStudent
                              ? `${12 - idx * 2} students interested`
                              : `${45 + idx * 10} similar bootcamps`}
                          </Typography>
                        </Box>
